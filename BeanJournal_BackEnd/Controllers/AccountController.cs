    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordDTO forgotPasswordDto)
        {
            ...
            if (user != null)
            {
                ...
                var domain = "https://beanjournal.vercel.app/reset-password";
  ...
  >
    ...
    >
      <div style=""text-align: center; padding: 20px; background-color: white"">
        ...
        >
          <img
            src=""https://res.cloudinary.com/dp34so8og/image/upload/v1725983655/beanjournallogo_dyyzat.png""
            alt=""Logo""
            style=""width: 80px; height: auto; margin-right: 16px""
          />
          <span style=""font-size: 3.5rem; font-weight: 600; color: #db2777""
            >Bean Journal</span
          >
        </div>
      </div>
...
";
                var message = new Message([user.Email!], "Reset password link", htmlMail);
                await _emailService.SendEmail(message);

                return Ok($"A password change request has been sent to {user.Email}.Please open your email & click on the link");
            }
            return BadRequest("User not found");
        }
